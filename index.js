...
try {
    ...
    mainPromise = utils
        ...
        Fetch('https://raw.githubusercontent.com/RqzaX040/Fca-Hokai-Remake/main/package.json').then(async (/** @type {{ body: { toString: () => string; }; }} */res) => {
            const localVersion = JSON.parse(readFileSync('./node_modules/fca-rqzax/package.json')).version;
                if (Number(localVersion.replace(/\./g,"")) < Number(JSON.parse(res.body.toString()).version.replace(/\./g,"")) ) {
                    log.warn("[ FCA ] •",getText(Language.NewVersionFound,JSON.parse(readFileSync('./node_modules/fca-rqzax/package.json')).version,JSON.parse(res.body.toString()).version));
                    if (globalThis.Fca.Require.FastConfig.AutoUpdate == true) { 
                        log.warn("[ FCA ] •",Language.AutoUpdate);
                            try {
                                execSync('npm install fca-rqzax@latest', { stdio: 'inherit' });
                                ...
                                try {
                                    require.resolve('rqzax-sp');
                                }
                                catch (e) {
                                    logger.Normal(Language.InstallSupportTool);
                                        execSync('npm install rqzax-sp@latest', { stdio: 'inherit' });
                                    process.exit(1);
                                }
                                    var fcasp = require('rqzax-sp');
                                try {
                                    fcasp.onError()
                                }
                                catch (e) {
